...
public:
	...
	static void Print(clsDate& Date, const string& Prameter = "/", const bool EndLine = true)
	{
		cout << Date.Day << Prameter << Date.Month << Prameter << Date.Year;

		if (EndLine)
			cout << endl;
	}

	void Print(const string& Prameter = "/", const bool& EndLine = true)
	{
		Print(*this, Prameter, EndLine);
	}
	...
	int GetDifferenceInDays(const clsDate& Date2, const bool& IncludeEndDay = false)
	{
	...
	static string Format(clsDate& Date, const string& DateFormat = "dd/mm/yyyy")
	{
		string FormattedDateString = "";

		FormattedDateString = clsString::ReplaceWord
		(DateFormat, "dd", to_string(Date.Day));

		FormattedDateString = clsString::ReplaceWord
		(FormattedDateString, "mm", to_string(Date.Month));

		FormattedDateString = clsString::ReplaceWord
		(FormattedDateString, "yyyy", to_string(Date.Year));

		return  FormattedDateString;
	}

	string Format(const string& DateFormat = "dd/mm/yyyy")
	{
